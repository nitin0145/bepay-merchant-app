import { X } from 'lucide-react';
import { useState, useEffect } from 'react';

interface PaymentLinksFilterDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  activeFilters: {
    status?: string;
  };
  onApplyFilters: (filters: {
    status?: string;
  }) => void;
}

export default function PaymentLinksFilterDrawer({
  isOpen,
  onClose,
  activeFilters,
  onApplyFilters,
}: PaymentLinksFilterDrawerProps) {
  const [status, setStatus] = useState(activeFilters.status || '');

  // Mock states for UI only (from Figma)
  const [fixedRate, setFixedRate] = useState('');
  const [feePaidBy, setFeePaidBy] = useState('');
  const [payinAddress, setPayinAddress] = useState('');
  const [payingHash, setPayingHash] = useState('');
  const [outcomeCurrency, setOutcomeCurrency] = useState('');

  // Sync state when drawer opens or active filters change
  useEffect(() => {
    if (isOpen) {
      setStatus(activeFilters.status || '');
    }
  }, [isOpen, activeFilters]);

  const handleApply = () => {
    onApplyFilters({
      status: status || undefined,
    });
    onClose();
  };

  const handleClearAll = () => {
    setStatus('');
    setFixedRate('');
    setFeePaidBy('');
    setPayinAddress('');
    setPayingHash('');
    setOutcomeCurrency('');
    onApplyFilters({});
    onClose();
  };

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      {/* Backdrop */}
      <div
        className="absolute inset-0 bg-black/30 backdrop-blur-xs transition-opacity duration-300"
        onClick={onClose}
      />

      {/* Drawer Container */}
      <div 
        role="dialog"
        aria-modal="true"
        aria-labelledby="pl-filter-title"
        className="absolute inset-y-0 right-0 pl-10 max-w-full flex"
      >
        <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col justify-between border-l border-black/5 animate-slide-in">
          {/* Header */}
          <div className="p-6 border-b border-black/5 flex justify-between items-center bg-[#F8F8FA]">
            <div>
              <h2 id="pl-filter-title" className="text-base font-extrabold text-black tracking-tight">Filters</h2>
              <p className="text-[10px] text-muted-foreground font-semibold mt-0.5">
                Refine payment links
              </p>
            </div>
            <button
              onClick={onClose}
              className="h-8 w-8 rounded-full border border-black/5 flex items-center justify-center text-muted-foreground hover:text-black hover:bg-black/5 transition-all"
            >
              <X className="h-4 w-4" />
            </button>
          </div>

          {/* Body */}
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {/* Status */}
            <div className="space-y-2">
              <label className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground">
                Payment Status
              </label>
              <select
                value={status}
                onChange={(e) => setStatus(e.target.value)}
                className="w-full h-11 px-4 text-xs font-semibold rounded-xl border border-black/5 bg-white text-black focus:outline-hidden focus:border-black transition-all appearance-none cursor-pointer"
                style={{
                  backgroundImage: `url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3e%3cpolyline points='6 9 12 15 18 9'%3e%3c/polyline%3e%3c/svg%3e")`,
                  backgroundRepeat: 'no-repeat',
                  backgroundPosition: 'right 16px center',
                  backgroundSize: '12px',
                }}
              >
                <option value="">Choose</option>
                <option value="ACTIVE">Active</option>
                <option value="PAID">Paid</option>
                <option value="EXPIRED">Expired</option>
              </select>
            </div>

            {/* Fixed Rate */}
            <div className="space-y-2">
              <label className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground">
                Fixed Rate
              </label>
              <select
                value={fixedRate}
                onChange={(e) => setFixedRate(e.target.value)}
                className="w-full h-11 px-4 text-xs font-semibold rounded-xl border border-black/5 bg-white text-black focus:outline-hidden focus:border-black transition-all appearance-none cursor-pointer"
                style={{
                  backgroundImage: `url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3e%3cpolyline points='6 9 12 15 18 9'%3e%3c/polyline%3e%3c/svg%3e")`,
                  backgroundRepeat: 'no-repeat',
                  backgroundPosition: 'right 16px center',
                  backgroundSize: '12px',
                }}
              >
                <option value="">Choose</option>
                <option value="YES">Yes</option>
                <option value="NO">No</option>
              </select>
            </div>

            {/* Fee Paid By User */}
            <div className="space-y-2">
              <label className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground">
                Fee Paid By User
              </label>
              <select
                value={feePaidBy}
                onChange={(e) => setFeePaidBy(e.target.value)}
                className="w-full h-11 px-4 text-xs font-semibold rounded-xl border border-black/5 bg-white text-black focus:outline-hidden focus:border-black transition-all appearance-none cursor-pointer"
                style={{
                  backgroundImage: `url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3e%3cpolyline points='6 9 12 15 18 9'%3e%3c/polyline%3e%3c/svg%3e")`,
                  backgroundRepeat: 'no-repeat',
                  backgroundPosition: 'right 16px center',
                  backgroundSize: '12px',
                }}
              >
                <option value="">Choose</option>
                <option value="MERCHANT">Merchant</option>
                <option value="USER">User</option>
              </select>
            </div>

            {/* Payin address */}
            <div className="space-y-2">
              <label className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground">
                Payin address
              </label>
              <input
                type="text"
                value={payinAddress}
                onChange={(e) => setPayinAddress(e.target.value)}
                placeholder="Enter Address"
                className="w-full h-11 px-4 text-xs font-semibold rounded-xl border border-black/5 bg-white text-black focus:outline-hidden focus:border-black transition-all"
              />
            </div>

            {/* Paying hash */}
            <div className="space-y-2">
              <label className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground">
                Paying hash
              </label>
              <input
                type="text"
                value={payingHash}
                onChange={(e) => setPayingHash(e.target.value)}
                placeholder="Enter hash"
                className="w-full h-11 px-4 text-xs font-semibold rounded-xl border border-black/5 bg-white text-black focus:outline-hidden focus:border-black transition-all"
              />
            </div>

            {/* Outcome currency */}
            <div className="space-y-2">
              <label className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground">
                Outcome currency
              </label>
              <select
                value={outcomeCurrency}
                onChange={(e) => setOutcomeCurrency(e.target.value)}
                className="w-full h-11 px-4 text-xs font-semibold rounded-xl border border-black/5 bg-white text-black focus:outline-hidden focus:border-black transition-all appearance-none cursor-pointer"
                style={{
                  backgroundImage: `url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3e%3cpolyline points='6 9 12 15 18 9'%3e%3c/polyline%3e%3c/svg%3e")`,
                  backgroundRepeat: 'no-repeat',
                  backgroundPosition: 'right 16px center',
                  backgroundSize: '12px',
                }}
              >
                <option value="">Choose</option>
                <option value="USDC">USDC</option>
                <option value="USDT">USDT</option>
              </select>
            </div>
          </div>

          {/* Footer Actions */}
          <div className="p-6 border-t border-black/5 bg-[#F8F8FA] grid grid-cols-2 gap-4">
            <button
              onClick={handleClearAll}
              className="h-11 border border-black/5 bg-white text-black hover:bg-black/5 active:scale-[0.98] transition-all text-xs font-bold rounded-xl flex items-center justify-center shadow-xs"
            >
              Clear All
            </button>
            <button
              onClick={handleApply}
              className="h-11 bg-black text-white hover:bg-black/90 active:scale-[0.98] transition-all text-xs font-bold rounded-xl flex items-center justify-center shadow-xs"
            >
              Apply Filter
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}
